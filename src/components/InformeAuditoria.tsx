...
import {
  TerapiaClassificationSection,
  InterconsultasSection,
  PracticasExcluidasSection,
  EndoscopiasSection,
  PracticasAmbulatoriasSection
} from './AuditSections';
...
export function InformeAuditoria({ resultado, auditoriaId }: Props) {
  ...
  const InformePill = ({ ok }: { ok: boolean }) => (
    <span
      className={`px-2 py-0.5 rounded-full text-xs font-semibold border ${ok ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'
        }`}
    >
  ...
  return (
    <>
      <div className="space-y-8">
        ...
        <TerapiaClassificationSection resultadoTerapia={resultado.resultadoTerapia} />

        {/* Interconsultas */}
        <InterconsultasSection interconsultas={resultado.interconsultas || []} />

        {/* Prácticas Excluidas */}
        <PracticasExcluidasSection practicas={resultado.practicasExcluidas || []} />

        {/* Endoscopías */}
        <EndoscopiasSection endoscopias={resultado.endoscopias || []} />

        {/* Prácticas Ambulatorias */}
        <PracticasAmbulatoriasSection practicas={resultado.practicasAmbulatorias || []} />
        ...
        {resultado.resultadosFoja && (
          <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-xl p-8">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div
                className={`p-4 rounded-lg ${resultado.resultadosFoja.bisturi_armonico === 'SI' ? 'bg-red-100' : 'bg-green-50'
                  }`}
              >
        ...
        <div className="bg-gradient-to-r from-gray-900 to-gray-800 rounded-xl shadow-lg p-8 text-white">
          ...
          <div className="flex items-center gap-4 mb-4">
            ...
            <span
              className={`px-4 py-2 rounded-full font-bold ${resultado.pacienteInternado
        ...
        {notification && (
          <div className="fixed bottom-6 right-6 z-50 animate-slide-in">
            <div
              className={`px-6 py-4 rounded-lg shadow-2xl flex items-center gap-3 ${notification.type === 'success' ? 'bg-green-600 text-white' : 'bg-red-600 text-white'
                }`}
            >
